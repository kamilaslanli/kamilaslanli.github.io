...
$allow_types=array("mp3","ogg","m4a","oga","wav","acc");
			...
			If($_FILES['file']['name'][$i]) {
				...
				// Check if the file already exists on the server..
					...
					If(move_uploaded_file($_FILES['file']['tmp_name'][$i],$folder.$file_name[$i].".".$file_ext[$i])) {
                                                ...
                                                mysql_query("insert into `mesaj` set `uid`='".$rom['id']."',`nik`='".$rom['user']."',`text`='[audio]".$full_url.$file_name[$i].".".$file_ext[$i]."[/audio]',`vaxt`='".time()."'");						
...
<hr/>
	...
	<?For($i=0;$i <= $file_uploads-1;$i++) {?>
		...
		<input class="form-control input-lg" type="file" name="file[]" accept="audio/*" capture="microphone"/>
