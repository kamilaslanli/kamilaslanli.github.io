...
echo '<title>Sekil Gonder</title>';
echo '<div class="line-menu">Sekil Gonder</div><div class="menu-border"><div class="menu3">';
...
$allow_types=array("jpg","jpeg","gif","png","ico");
			...
			If($_FILES['file']['name'][$i]) {
				...
				// Check if the file already exists on the server..
					...
					If(move_uploaded_file($_FILES['file']['tmp_name'][$i],$folder.$file_name[$i].".".$file_ext[$i])) {
                                                ...
                                                mysql_query("insert into `mesaj` set `uid`='".$rom['id']."',`nik`='".$rom['user']."',`text`='[url=".$full_url.$file_name[$i].".".$file_ext[$i]."][img=".$full_url.$file_name[$i].".".$file_ext[$i]."][/url]',`vaxt`='".time()."'");						
...
<hr/>
	...
	<?}?>
			...
			<input type="submit" value="Gonder" style="width:auto;"/>
			</span>
                      </div>
...
echo '<title>Video Gonder</title>';
echo '<div class="line-menu">Video Gonder</div><div class="menu-border"><div class="menu3">';
// Max size PER file in KB
$max_file_size="3072";
...
// Types of files that are acceptiable for uploading. Keep the array structure.
$allow_types=array("3gp","avi","mp4","mov","m4a");
		...
		For($i=0; $i <= $file_uploads-1; $i++) {
			...
			If($_FILES['file']['name'][$i]) {
				...
				// Check if the file already exists on the server..
					...
					If(move_uploaded_file($_FILES['file']['tmp_name'][$i],$folder.$file_name[$i].".".$file_ext[$i])) {
                                                 mysql_query("insert into `mesaj` set `uid`='".$rom['id']."',`nik`='".$rom['user']."',`text`='[vid]".$full_url.$file_name[$i].".".$file_ext[$i]."[/vid]',`vaxt`='".time()."'");	
...
<hr/>
	...
	<?For($i=0;$i <= $file_uploads-1;$i++) {?>
		...
		<input class="form-control input-lg" type="file" name="file[]" accept="video/*"/>
